...
class MediaLibrary:
    ...
    def readCache(self):
        cachefile = os.path.normpath(unicode(self.config.get('cacheFile')))
    ...
    def writeCache(self):
        ...
        cachefile = os.path.normpath(unicode(self.config.get('cacheFile')))
        if (not self.cacheBackedUp and os.path.exists(cachefile)):
            shutil.copyfile(cachefile, cachefile + '.prev')
            self.cacheBackedUp = True
        ElementTree(self.toXml()).write(cachefile, 'UTF-8')
    
    
    def readMedia(self):
        mediadir = os.path.normpath(unicode(self.config.get('mediaDir')))
        verbose = self.config.getboolean('verbose')
        skipExtensions = map(lambda x: '.' + x.lower().strip(), self.config.get('skipExtensions').split(','))
        
        self.outputWrapper.logNormal(u'Reading existing metadata from [' + mediadir + ']')
        numfiles = 0
        for root, dirs, files in os.walk(mediadir):
            for filename in files:
                ...
                numfiles += 1
                if (verbose):
                    self.outputWrapper.logNormal(u'\tProcessed: ' + os.path.join(root, filename))
        self.outputWrapper.logNormal(u'Read [' + unicode(numfiles) + '] media files')            
    ...
    def printLibrary(self):
        for artist in self.mediaLibrary:
            self.outputWrapper.logNormal(artist + ' (' + ', '.join(map(lambda pair: pair[0], self.mediaLibrary[artist]['tags'] or [])) + ')')
            for album in self.mediaLibrary[artist]['albums']:
                self.outputWrapper.logNormal(u'\t' + album)
                for track in self.mediaLibrary[artist]['albums'][album]['tracks']:
                    self.outputWrapper.logNormal(u'\t\t' + track + ' (' + ', '.join(map(lambda pair: pair[0], self.mediaLibrary[artist]['albums'][album]['tracks'][track]['tags'] or [])) + ')')                        
                ...
                if (artistDict['tags'] is not None):
                    if (len(artistDict['tags']) == 0):
                        ...
                        for tagpair in sorted(artistDict['tags']):
                            SubElement(artistElement, 'tag', weight=unicode(tagpair[1])).text = tagpair[0]
                        ...
                        if (trackDict['tags'] is not None):
                            if (len(trackDict['tags']) == 0):
                                ...
                                for tagpair in sorted(trackDict['tags']):
                                    SubElement(trackElement, 'tag', weight=unicode(tagpair[1])).text = tagpair[0]

                        numtracks += 1
                    numalbums += 1
                numartists += 1
            self.outputWrapper.logNormal(u'Serialized [' + unicode(numartists) + '] artists, [' + unicode(numalbums) + '] albums, and [' + unicode(numtracks) + '] tracks to XML')
            
            localTagsElement = SubElement(libraryElement, 'localTags')
            for tagpair in sorted(self.localTagLibrary.items(), key=lambda x: x [1]['localhits'], reverse=True):
                tag = tagpair[0]
                tagdict = tagpair[1]
                SubElement(localTagsElement, 'tag',
                        hits=unicode(tagdict['localhits'] or 0),
                        lastFmHits=unicode(tagdict['lasthits'] or 0),
                        lastFmTags=unicode(','.join(tagdict['lastfmkeys'] if 'lastfmkeys' in tagdict else []))).text = tagdict['disp']

            lastTagsElement = SubElement(libraryElement, 'lastFmTags')
            numtags = 0
            for tag in sorted(self.lastTagLibrary):
                SubElement(lastTagsElement, 'tag', hits=unicode(self.lastTagLibrary[tag] or 0)).text = tag
                numtags += 1

            self.outputWrapper.logNormal(u'Serialized [' + unicode(numtags) + '] lastFM tags to XML')
    
            return libraryElement
        except Exception, err:
            raise Exception(u'Could not serialize the XML cache data: ' + unicode(err)), None, sys.exc_info()[2]
        ...
        try:
            ...
            for artistElement in artistsElement.findall('artist'):
                ...
                if (nameElement is None):
                    self.outputWrapper.logError(u'Missing name element on [' + artistElement.tag + ']')
                    continue                
                artist = unicode(nameElement.text.lower() if ignoreCase else nameElement.text)
                ...
                for albumElement in artistElement.findall('album'):
                    ...
                    if (nameElement is None):
                        self.outputWrapper.logError(u'Missing name element on [' + albumElement.tag + ']')
                        continue
                    album = unicode(nameElement.text.lower() if ignoreCase else nameElement.text)
                    
                    for trackElement in albumElement.findall('track'):
                        nameElement = trackElement.find('name')
                        if (nameElement is None):
                            self.outputWrapper.logError(u'Missing name element on [' + trackElement.tag + ']')
                            continue
                        track = unicode(nameElement.text.lower() if ignoreCase else nameElement.text)                        
                    ...
                    numalbums += 1
                numartists += 1
            self.outputWrapper.logNormal(u'Loaded [' + unicode(numartists) + '] artists, [' + unicode(numalbums) + '] albums, and [' + unicode(numtracks) + '] cached tracks')
            ...
            raise Exception(u'Could not deserialize the XML cache data, possibly corrupted: ' + unicode(err)), None, sys.exc_info()[2]
    ...
    def fetchArtistTags(self, lastfm):
        ...
        self.outputWrapper.logNormal(u'Fetching artist tags from LastFM')
        for artist in sorted(self.mediaLibrary):
            ...
            if (verbose):
                self.outputWrapper.logNormal(u'\tFetched [' + artist + '] (' + (', '.join(map(lambda pair: pair[0], tagpairs)) if tagpairs is not None else '') + ')')
            time.sleep(niceness)
    ...
    def fetchTrackTags(self, lastfm):
        ...
        self.outputWrapper.logNormal(u'Fetching track tags from LastFM')
        for artist in sorted(self.mediaLibrary):
            if (artist in self.artistSkipList):
                continue
            for album in sorted(self.mediaLibrary[artist]['albums']):
                for track in sorted(self.mediaLibrary[artist]['albums'][album]['tracks']):
                    ...
                    if (tagpairs is not None):
                        map(self.addToLastFMTagLibrary, map(lambda pair: pair[0], tagpairs))
                    if (verbose):
                        self.outputWrapper.logNormal(u'\tFetched [' + artist + ':' + track + '] (' + (', '.join(map(lambda pair: pair[0], tagpairs)) if tagpairs is not None else '') + ')')
                    time.sleep(niceness)
    ...
    def fetchTagStats(self, lastfm):
        ...
        if (toptags is None or len(toptags) == 0):
            self.outputWrapper.logError(u'Could not retrieve tag counts from lastFM')
    ...
    def updateTags(self):
        ...
        if (touchedFields is None or len(touchedFields) == 0):
            self.outputWrapper.logError(u'Perhaps you should configure a destination field...')
            return
        
        self.loadSynonyms()
        self.generateLocalTags()
        
        self.outputWrapper.logNormal(u'Updating tags in [' + mediadir + ']')
        numfiles = 0
        for root, dirs, files in os.walk(mediadir):
            for filename in files:
                try:
                    ...
                    artistTags = []
                    trackTags = []
                    for artist in map(string.lower, metadata['artists']) if ignoreCase else metadata['artists']:                                        
                        if (artist in self.artistSkipList): 
                            continue
                        if (artist not in self.mediaLibrary or
                            ...
                            self.outputWrapper.logError(u'Entry not found in library: [' + artist + '][' + album + '][' + track + ']')
                            continue
                    ...
                    if (self.mediaHelper.updateTags(os.path.join(root, filename), tagPayload)):                                                                      
                        numfiles += 1
                        if (verbose):
                            self.outputWrapper.logNormal(u'\tUpdated: ' + os.path.join(root, filename))
                    elif (verbose):
                        self.outputWrapper.logNormal(u'\tSkipped: ' + os.path.join(root, filename) + ' (nothing to update)')
                except Exception, err:
                    self.outputWrapper.logError(u'\tFailed to update: ' + os.path.join(root, filename) + ' (' + unicode(err) + ')')
                    pass
        self.outputWrapper.logNormal(u'Updated [' + unicode(numfiles) + '] media files')
        #if (verbose):
    ...
    def loadSynonyms(self):
        ...
        if (not os.path.exists(synfile) or not os.access(synfile, os.R_OK)):
            self.outputWrapper.logError(u'Synonyms file either does not exist or cannot be accessed [' + synfile + ']')
        ...
        for line in fileinput.input(synfile):
            ...
            synline = line.split('\t')
            if (len(synline) < 2):
                self.outputWrapper.logError(u'Invalid synonym file line: ' + line)
                continue
        ...
        if (self.config.getboolean('verbose')):
            self.outputWrapper.logNormal(u'Loaded [' + unicode(len(self.synonyms.keys())) + '] tag synonyms')           
    ...
    def loadSkipList(self, strInFile):
        ...
        if (not os.path.exists(strInFile) or not os.access(strInFile, os.R_OK)):
            self.outputWrapper.logError(u'SkipList file either does not exist or cannot be accessed [' + strInFile + ']')
        ...
        for line in fileinput.input(strInFile):
            ...
            tmpSet.add(line)
        if (self.config.getboolean('verbose')):
            self.outputWrapper.logNormal(u'Loaded [' + unicode(len(tmpSet)) + '] skip list entries from [' + strInFile + ']')
        return tmpSet           
    ...
    def generateLocalTags(self):
        ...
        self.localTagLibrary['untagged artist'] = dict(disp=u'Untagged Artist', lastfmkeys=[], localhits=0, lasthits=0)
        self.localTagLibrary['untagged track'] = dict(disp=u'Untagged Track', lastfmkeys=[], localhits=0, lasthits=0)
    ...
    def printDistinctLocalTags(self):
        ...
        self.outputWrapper.logNormal(u'\nDistinct Update-stream Tags (most to least frequent, in your library): \n\t' + '\n\t'.join(map(lambda pair: pair[0] + ' (' + unicode(pair[1]) + ')', common.sortWeightedTagTuples(disptags))))
