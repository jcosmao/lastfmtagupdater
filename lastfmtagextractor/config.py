...
Usage:
    ...
    --gui        use a windowed gui for the output (requires Tk)
'''
    ...
    defaults = dict(
        ...
        logFile='lastfmtagextractor.log',
        mediadir='.',
        niceness='50',
        ignoreCase='true',
    ...
    def parseargs(self, argv=None):
        ...
        try:    
            ...
            print >> sys.stderr, os.path.basename(sys.argv[0]) + ': ' + unicode(err)
    ...
    def validFieldSet(self, option, configSet, validSet):
        ...
        if (len(configSet.difference(validSet)) > 0):
            raise Exception('One or more invalid fields were specified for option [' + option + ']: ' + unicode(configSet.difference(validSet)))                
